...
export class BitstarsinvComponent {
  ...
  constructor(private router: Router, private db: AngularFireDatabase, private snackBar: MatSnackBar, 
    ...
    // this.db.list('investments/bitstarsinv/comments').valueChanges().subscribe(
    //   (data:any) => {
    //     if (data) {
  ...
  addComment () {
      // this.db.database.ref('investments').child('bitstarsinv').child('comments').child(this.numberOfComments.toString()).set(
      //   {
