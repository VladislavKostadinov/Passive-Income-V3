...
export class F2hashComponent {
  ...
  constructor(private router: Router, private db: AngularFireDatabase, private snackBar: MatSnackBar, 
    ...
    this.db.list('cloud-mining/f2hash/comments').valueChanges().subscribe(
      (data:any) => {
        if (data) {
  ...
  addComment () {
      this.db.database.ref('cloud-mining').child('f2hash').child('comments').child(this.numberOfComments.toString()).set(
        {
